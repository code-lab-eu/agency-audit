...
class TestQC:
    ...
    async def test_mark_manual_review_jsonb_with_special_chars(self):
        """mark_for_manual_review should safely bind reason with quotes,
        backslashes, and newlines via json.dumps (no f-string injection)."""
        import json
        ...
        with patch("agency_audit.loop.qc.get_pool") as mock_get_pool:
            ...
            # reason with double-quote, backslash, and newline
            reason = 'found "injection" risk\\backslash\nnewline here'
            await mark_for_manual_review(42, reason, severity="error")

            assert mock_conn.execute.called
            call_args = mock_conn.execute.call_args
            # param $1 = reason (plain text)
            assert call_args.args[1] == reason
            # param $2 = JSONB array as json.dumps output
            jsonb_param = call_args.args[2]
            ...
            # Verify no fragment of an f-string pattern survived
